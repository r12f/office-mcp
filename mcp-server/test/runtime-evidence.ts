...
const endpoint = readOption('--endpoint') ?? 'http://127.0.0.1:8800/mcp';
const outputPath = resolve(readOption('--output') ?? '../artifacts/runtime-evidence.json');
...
try {
  ...
  if (!requestedSessionId && irmDocumentPath && waitForSessionMs > 0 && !selectSessionId(sessions, irmDocumentPath)) {
  ...
  const sessionId = requestedSessionId ?? selectSessionId(sessions, irmDocumentPath);
...
async function runWaitForSessionGate(documentPath: string, timeoutMs: number): Promise<Record<string, unknown>> {
  return await runGate('word.wait_for_requested_session', async () => {
    ...
    while (Date.now() - started <= timeoutMs) {
      ...
      const sessionId = selectSessionId(sessions, documentPath);
...
async function runAgentClientBridgeGate(sessionId: string): Promise<void> {
  await runGate('agent_client_stdio_bridge', async () => {
    ...
    const stdioTransport = new StdioClientTransport({ command: process.execPath, args: ['dist/src/cli.js', 'stdio'], cwd: process.cwd(), stderr: 'pipe' });
...
function runSmokeMode(mode: string, sessionId: string, extraArg?: string): string {
  const args = ['test/mcp-smoke.ts', endpoint, mode, sessionId];
  if (extraArg) args.push(extraArg);
  const raw = execFileSync(process.execPath, ['./node_modules/tsx/dist/cli.mjs', ...args], {
    cwd: process.cwd(),
    encoding: 'utf8',
...
function selectSessionId(sessions: Array<Record<string, unknown>>, documentPath?: string): string {
  if (documentPath) {
    const match = sessions.find((session) => sessionInfoMatchesDocumentPath(session, documentPath) && session.status === 'active');
    if (match?.session_id) return String(match.session_id);
    return '';
  }
  return String(sessions.find((session) => session.status === 'active')?.session_id ?? sessions[0]?.session_id ?? '');
}
