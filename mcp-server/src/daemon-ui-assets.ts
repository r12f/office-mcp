...
<html lang="en">
  ...
  <body>
    ...
    <main id="main" class="shell">
      ...
      <section class="daemon-details" aria-labelledby="daemonDetailsHeading">
        <h2 id="daemonDetailsHeading">Daemon Details</h2>
        <dl>
          <dt>Version</dt><dd id="daemonVersion">-</dd>
          <dt>Uptime</dt><dd id="daemonUptime">-</dd>
          <dt>Config</dt><dd><code id="configPath">Not configured</code><button class="icon-button" type="button" data-copy="configPath" aria-label="Copy Config Path">Copy</button></dd>
          <dt>Log</dt><dd><code id="logPath">Not configured</code><button class="icon-button" type="button" data-copy="logPath" aria-label="Copy Log Path">Copy</button></dd>
          <dt>Last Error</dt><dd id="lastError">None</dd>
          <dt>Stdio Bridge</dt><dd><code id="stdioBridgeCommand">office-mcp stdio</code><button class="icon-button" type="button" data-copy="stdioBridgeCommand" aria-label="Copy Stdio Bridge Command">Copy</button></dd>
        </dl>
      </section>
...
export const DAEMON_CONSOLE_JS = String.raw`(() => {
  ...
  function render(snapshot) {
    ...
    el('daemonVersion').textContent = snapshot.daemon.version || '-';
    el('daemonUptime').textContent = uptime(snapshot.daemon.uptime_ms || 0);
    el('configPath').textContent = snapshot.daemon.config_path || 'Not configured';
    el('logPath').textContent = snapshot.daemon.log_path || 'Not configured';
    el('lastError').textContent = snapshot.daemon.last_error || 'None';
    el('stdioBridgeCommand').textContent = 'office-mcp stdio';
    el('documents').innerHTML = renderDocumentGroups(snapshot.documents || {});
    el('clients').innerHTML = snapshot.clients.length ? table(['Client', 'Transport', 'Last Activity', 'In Flight'], snapshot.clients.map(clientRow)) : '<p class="empty">No clients connected. Copy the MCP endpoint or the stdio bridge command.</p>';
  ...
  async function copyText(text, button) { if (!text || text === 'Not configured') return; await navigator.clipboard.writeText(text); announce('Copied ' + text); button.textContent = 'Copied'; setTimeout(() => { button.textContent = 'Copy'; }, 1200); }
