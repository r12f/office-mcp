...
test('Office catalog registration script stages Office host manifests without registry mutation', () => {
  ...
  try {
    ...
    assert.match(excelManifest, /https:\/\/localhost:8766\/excel\/taskpane\.html\?v=0\.1\.10/);
...
test('Word task pane exposes product UI regions and accessible endpoint settings', () => {
  ...
  assert.match(js, /\{ label: 'Insert', tools: \['word\.insert_paragraph', 'word\.insert_image', 'word\.insert_table', 'word\.insert_page_break', 'word\.insert_list'\] \}/);
  assert.match(js, /\{ label: 'Edit', tools: \['word\.replace_text', 'word\.update_paragraph', 'word\.delete_range', 'word\.apply_formatting', 'word\.apply_style'\] \}/);
