...
1. **Command Console Panes**: layered abstract document/app panes connected by
   a command rail, operator nodes, and a single control reticle. This direction
   was selected because it communicates office work, local routing, and
   deliberate user control at 16 px tray size, 32 px ribbon size, catalog
   thumbnail size, title-bar size, and installer size without borrowing
...
identity mature and slightly futuristic through precise geometry, layered
control-console depth, restrained accent colors, and clear operator affordances
rather than decorative glow or neon styling.

## Palette

- `#0F172A`: desktop utility shell and high-contrast stroke.
- `#EAF2FF`: neutral document pane surface.
- `#2457D6`: command rail and routing accent.
- `#48D6A4`: automation/control pane accent.
- `#243B7A`: deep control-console pane.
- `#F8D84A`: active control node.

The palette is intentionally not a Microsoft Office app palette. Host-specific
Word and Excel colors remain UI accents only and are not the product mark.

## Minimum Sizes

- 16 px: tray/menu-bar glyph must keep the dark shell, command rail, and
  yellow control reticle legible enough to avoid reading as a blank square.
- 32 px: ribbon and Windows notification-area usage must show the control
  panes, command rail, and control reticle.
- 80 px and above: Office command icon and installer usage should show all pane,
  routing, operator-node, and dial details.
...
original abstract control console for documents, with the command rail and
control reticle as its signature elements.
