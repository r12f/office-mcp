...
function renderDocumentCard(doc, app) {
  ...
  return `<button class="row document-card ${esc(app)}" type="button" data-key-activate data-inspect='${attr(doc)}' aria-label="Inspect ${esc(label)} ${esc(status)}"><span class="document-card-title"><span class="state-dot ${esc(documentStateTone(doc.status))}" aria-hidden="true"></span><strong title="${esc(label)}">${esc(label)}</strong><span class="pill ${esc(documentStateTone(doc.status))}">${esc(title(status))}</span></span><span class="document-card-session" title="${esc(doc.session_id || '-')}">Session ${esc(middleTruncate(doc.session_id, 18))}</span><span class="document-card-meta"><span>Version ${esc(doc.host?.version || '-')}</span><span>${esc(doc.available_tool_count || 0)} tools</span><span>Queue ${esc(doc.queue_depth || 0)}</span></span><span class="document-card-meta"><span>Finished ${esc(metrics.finished)}</span><span>Failed ${esc(metrics.failed)}</span></span></button>`;
}
