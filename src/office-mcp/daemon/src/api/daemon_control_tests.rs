...
#[test]
fn status_json_delegates_to_runtime_status_reporter() {
    let controller = DaemonController::with_runtime_path(PathBuf::from("missing-runtime.json"));

    assert!(controller.status_json().contains("\"uiCommand\""));
}
