...
fn tool_metadata_classifies_app_category_and_side_effect() {
    ...
    let update_document_properties = tool_metadata("excel.update_document_properties")
        .expect("excel document properties update metadata");
    assert_eq!(update_document_properties.app, "excel");
    assert_eq!(update_document_properties.category, "Workbook");
    assert_eq!(
