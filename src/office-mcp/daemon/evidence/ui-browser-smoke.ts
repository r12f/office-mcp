...
async function main(): Promise<void> {
  ...
  try {
    ...
    await assertEval(cdp, '(() => { const text = document.querySelector("#documents .row.word")?.textContent || ""; return text.includes("Session ID") && text.includes("11111111-1111-4111-8111-111111111111") && !text.includes("11111111..."); })()', 'word document card shows full session id');
    ...
    await assertEval(cdp, '(() => { const session = document.querySelector("#documents .row.word .document-card-session")?.textContent || ""; return session.includes("Session ID") && session.includes("11111111-1111-4111-8111-111111111111") && !session.includes("Version") && !session.includes("tools") && !session.includes("Queue"); })()', 'word document card session row contains only session identity');
    ...
    await assertEval(cdp, '(() => { const node = document.querySelector("#lastError"); const style = getComputedStyle(node); return node.value.trim().length > 0 && node.closest("button") === null && document.querySelector("[data-copy=\\"lastError\\"]").getAttribute("aria-label") === "Copy last error" && style.whiteSpace !== "nowrap" && style.textOverflow !== "ellipsis" && style.userSelect !== "none" && style.overflowY !== "hidden"; })()', 'daemon details show selectable scrollable copyable last error state');
