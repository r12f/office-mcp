...
async function main(): Promise<void> {
  ...
  try {
    ...
    await assertEval(cdp, 'document.querySelector("#lastError").textContent.trim().length > 0 && document.querySelector("#lastError").closest("button") === null && document.querySelector("[data-copy=\\"lastError\\"]").getAttribute("aria-label") === "Copy last error" && getComputedStyle(document.querySelector("#lastError")).whiteSpace !== "nowrap"', 'daemon details show selectable copyable last error state');
