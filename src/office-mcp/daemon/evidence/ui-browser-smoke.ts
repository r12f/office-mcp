...
async function main(): Promise<void> {
  ...
  try {
    ...
    await assertEval(cdp, '(() => { const dot = document.querySelector("#documents .row.word .document-card-title .state-dot"); const label = document.querySelector("#documents .row.word .document-card-title .pill"); return dot && label && label.textContent.trim() === "Active" && dot.getBoundingClientRect().width >= 6 && dot.compareDocumentPosition(label) & Node.DOCUMENT_POSITION_FOLLOWING; })()', 'word document card shows a stable state marker next to the state label');
    await assertEval(cdp, '(() => { const text = document.querySelector("#documents .row.word .document-card-session code")?.textContent || ""; return text.includes("...") && text.length < "11111111-1111-4111-8111-111111111111".length && !text.includes("11111111-1111-4111-8111-111111111111"); })()', 'word document card shows compact middle-truncated session id');
    await assertEval(cdp, '(() => { const card = document.querySelector("#documents .row.word"); const title = card?.querySelector(".document-card-title")?.getBoundingClientRect(); const session = card?.querySelector(".document-card-session")?.getBoundingClientRect(); const meta = card?.querySelector(".document-card-meta")?.getBoundingClientRect(); return title && session && meta && session.top > title.bottom - 1 && meta.top > session.bottom - 1; })()', 'word document card keeps session id on its own row');
    await assertEval(cdp, '(() => { const session = document.querySelector("#documents .row.word .document-card-session")?.textContent || ""; return !session.includes("Session ID") && session.includes("...") && !session.includes("11111111-1111-4111-8111-111111111111") && !session.includes("Version") && !session.includes("tools") && !session.includes("Queue"); })()', 'word document card session row contains only compact session identity without a redundant label');
    ...
    await assertEval(cdp, '(() => { const uptime = document.querySelector("#documents .row.word .document-card-uptime"); const card = document.querySelector("#documents .row.word")?.getBoundingClientRect(); const rect = uptime?.getBoundingClientRect(); return uptime && /\\d+(\\.\\d+)?[sm]$/.test(uptime.textContent.trim()) && card && rect && rect.right <= card.right + 1 && rect.bottom <= card.bottom + 1 && rect.right > card.right - 80; })()', 'word document card shows compact session uptime in the lower right');
    ...
    await cdp.send('Runtime.evaluate', { expression: '[...document.querySelectorAll("#documents .row")].at(-1).focus()' });
    await pressKey(cdp, 'ArrowUp', 38);
    await assertEval(cdp, 'document.activeElement === [...document.querySelectorAll("#documents .row")].at(-2)', 'document row ArrowUp moves focus to previous row');
    await pressKey(cdp, 'End', 35);
    await assertEval(cdp, 'document.activeElement === [...document.querySelectorAll("#documents .row")].at(-1)', 'document row End moves focus to last row');
    await pressKey(cdp, 'Home', 36);
    await assertEval(cdp, 'document.activeElement === document.querySelector("#documents .row")', 'document row Home moves focus to first row');
    await pressKey(cdp, 'PageDown', 34);
    await assertEval(cdp, 'document.activeElement === [...document.querySelectorAll("#documents .row")].at(-1)', 'document row PageDown moves focus toward the end');
    ...
    await pressKey(cdp, 'ArrowUp', 38);
    await assertEval(cdp, 'document.activeElement === document.querySelector("#history tr[data-inspect]")', 'history row ArrowUp moves focus to previous row');
    ...
    await assertEval(cdp, '[...document.querySelectorAll(".details dt > span:first-child")].some((node) => node.textContent.trim() === "Config file") && ![...document.querySelectorAll(".details dt > span:first-child")].some((node) => node.textContent.trim() === "Config")', 'daemon details label config as config file path');
    ...
    await assertEval(cdp, 'document.querySelector("[data-copy=\\"configPath\\"]").getAttribute("aria-label") === "Copy config path" && document.querySelector("#configPath").closest("button") === null && document.querySelector("#configPath").closest(".detail-path-value") !== null && document.querySelector("[data-copy=\\"configPath\\"]").closest("dt") !== null && document.querySelector("[data-open-diagnostic=\\"config\\"]").closest("dt") !== null', 'config path is selectable text with compact header actions');
    await assertEval(cdp, 'document.querySelector("[data-copy=\\"logPath\\"]").getAttribute("aria-label") === "Copy log path" && document.querySelector("#logPath").closest("button") === null && getComputedStyle(document.querySelector("#logPath")).userSelect !== "none" && getComputedStyle(document.querySelector("#logPath")).whiteSpace !== "nowrap" && getComputedStyle(document.querySelector("#logPath")).textOverflow !== "ellipsis" && document.querySelector("[data-copy=\\"logPath\\"]").closest("dt") !== null && document.querySelector("[data-open-diagnostic=\\"log\\"]").closest("dt") !== null', 'log path is selectable text with compact header actions');
    ...
    await assertEval(cdp, '(() => { const node = document.querySelector("#lastError"); return node.value.split(String.fromCharCode(10)).length >= 3 && node.value.includes("Open the daemon log file"); })()', 'daemon details expose multiline last error text');
    await assertEval(cdp, 'document.querySelector("#lastError").closest("button") === null && document.querySelector("[data-copy=\\"lastError\\"]").getAttribute("aria-label") === "Copy last error"', 'daemon details keep last error selectable with separate copy button');
    await assertEval(cdp, '(() => { const style = getComputedStyle(document.querySelector("#lastError")); return style.whiteSpace !== "nowrap" && style.textOverflow !== "ellipsis" && style.userSelect !== "none" && style.overflowY !== "hidden"; })()', 'daemon details avoid clipped last error styling');
    ...
    await assertEval(cdp, '[...document.querySelectorAll("#toolAccessMode [data-access-mode]")].map((button) => button.dataset.accessMode).join(",") === "read,write,all"', 'global tool access panel exposes read write all modes');
    await assertEval(cdp, 'document.querySelector("#toolAccessList [data-tool-app=word] summary")?.textContent.includes("Word") && document.querySelector("#toolAccessList [data-tool-category=Range] summary")?.textContent.includes("Range")', 'global tool access groups tools by app and category');
    await assertEval(cdp, 'document.querySelector(\'#toolAccessList [data-tool-name="word.get_text"]\') !== null && document.querySelector(\'#toolAccessList [data-tool-name="excel.read_range"]\') !== null && document.querySelector(\'#toolAccessList [data-tool-name="powerpoint.list_slides"]\') !== null', 'global tool access lists word excel and powerpoint tools');
    await cdp.send('Runtime.evaluate', { expression: 'document.querySelector("#toolAccessMode [data-access-mode=read]").click()' });
    await waitFor(cdp, 'window.__OFFICE_MCP_UI__?.daemon?.tool_access_policy?.access_mode === "read"');
    await assertEval(cdp, 'document.querySelector("#toolAccessMode [data-access-mode=read]").getAttribute("aria-checked") === "true"', 'global tool access read mode updates UI state');
    await cdp.send('Runtime.evaluate', { expression: 'document.querySelector("#toolAccessList [data-tool-app=powerpoint] .tool-access-toggle").click()' });
    await waitFor(cdp, 'window.__OFFICE_MCP_UI__?.daemon?.tool_access_policy?.disabled_apps?.includes("powerpoint")');
    await assertEval(cdp, 'document.querySelector("#toolAccessList [data-tool-app=powerpoint]").open', 'app toggle does not collapse global tool access app group');
    await cdp.send('Runtime.evaluate', { expression: 'document.querySelector(\'#toolAccessList [data-tool-name="word.update_table"] .tool-access-toggle\').click()' });
