...
async function main(): Promise<void> {
  ...
  try {
    ...
    await assertEval(cdp, '(() => { const session = document.querySelector("#documents .row.word .document-card-session")?.textContent || ""; return session.includes("Session") && !session.includes("Version") && !session.includes("tools") && !session.includes("Queue"); })()', 'word document card session row contains only session identity');
