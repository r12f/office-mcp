...
async function main(): Promise<void> {
  ...
  try {
    ...
    await waitFor(cdp, 'document.querySelector("#documents .row.word").getAttribute("aria-expanded") === "true"');
    await assertEval(cdp, 'document.querySelector("#documents .doc-history:not([hidden])") !== null && document.querySelector("#documents .doc-history:not([hidden])").textContent.includes("word")', 'expanded document detail shows recent commands');
    await waitFor(cdp, 'document.querySelector("#inspector").textContent.includes("Runtime Evidence.docx")');
    await cdp.send('Runtime.evaluate', { expression: 'document.querySelector("#documents .row.word").click()' });
    await waitFor(cdp, 'document.querySelector("#documents .row.word").getAttribute("aria-expanded") === "false"');
    await assertEval(cdp, 'document.querySelector("#documents .doc-history[hidden]") !== null', 'document detail collapses after second click');
...
async function assertTaskpane(cdp: CdpClient, pageDiagnostics: PageDiagnostics, url: string, hostName: string, minimumToolGroups: number): Promise<void> {
  ...
  await assertEval(cdp, 'document.querySelector("#settingsToggle").getAttribute("aria-label") === "Open Settings" && document.querySelector("#settingsToggle").getAttribute("title") === "Open Settings"', `${hostName} taskpane settings button has accessible name and tooltip`);
  await assertEval(cdp, '(() => { const buttons = document.querySelectorAll(".metadata-copy"); const sessionTitle = buttons[0]?.getAttribute("title"); const daemonTitle = buttons[1]?.getAttribute("title"); return Boolean(sessionTitle && sessionTitle !== "-" && daemonTitle && daemonTitle !== "-"); })()', `${hostName} taskpane metadata copy buttons have useful tooltips`);
  ...
  await assertEval(cdp, '(() => { const buttons = document.querySelectorAll(".metadata-copy"); const sessionButton = buttons[0]; const daemonButton = buttons[1]; return sessionButton.title.length >= sessionButton.textContent.trim().length && daemonButton.title.length >= daemonButton.textContent.trim().length; })()', `${hostName} taskpane metadata copy tooltips are at least as complete as visible metadata`);
  ...
  await assertEval(cdp, 'document.querySelector("#settingsToggle").getAttribute("aria-label") === "Close Settings" && document.querySelector("#settingsToggle").getAttribute("title") === "Close Settings"', `${hostName} taskpane settings tooltip follows expanded state`);
  await assertEval(cdp, '(() => { const summary = document.querySelector(".summary-panel"); const settings = document.querySelector("#settingsPanel"); const task = document.querySelector(".current-task-panel"); const summaryRect = summary.getBoundingClientRect(); const settingsRect = settings.getBoundingClientRect(); const taskRect = task.getBoundingClientRect(); return settingsRect.height > 0 && settingsRect.top >= summaryRect.top && settingsRect.bottom <= summaryRect.bottom && taskRect.top - summaryRect.bottom <= 12; })()', `${hostName} expanded settings stays inside summary flow without a detached block`);
  await assertEval(cdp, '(() => { const summary = document.querySelector(".summary-panel"); const children = [...summary.children].filter((child) => getComputedStyle(child).display !== "none"); const rects = children.map((child) => child.getBoundingClientRect()).filter((rect) => rect.height > 0); return rects.every((rect, index) => index === 0 || (rect.top >= rects[index - 1].bottom && rect.top - rects[index - 1].bottom <= 12)); })()', `${hostName} expanded summary content has compact non-overlapping vertical spacing`);
  await assertEval(cdp, '(() => { const panels = [...document.querySelectorAll(".panel")].map((panel) => panel.getBoundingClientRect()); return panels.every((rect, index) => index === 0 || rect.top >= panels[index - 1].bottom); })()', `${hostName} taskpane panels do not overlap after settings opens`);
