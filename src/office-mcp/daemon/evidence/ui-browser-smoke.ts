...
async function main(): Promise<void> {
  ...
  try {
    ...
    await assertEval(cdp, '(() => { const label = document.querySelector("#documents .row.word .document-card-session span")?.getBoundingClientRect(); const value = document.querySelector("#documents .row.word .document-card-session code")?.getBoundingClientRect(); return label && value && value.top > label.bottom - 1; })()', 'word document card places session id value on a dedicated line');
    ...
    await assertEval(cdp, '[...document.querySelectorAll(".details dt")].some((node) => node.textContent.trim() === "Config file") && ![...document.querySelectorAll(".details dt")].some((node) => node.textContent.trim() === "Config")', 'daemon details label config as config file path');
    ...
    await assertEval(cdp, 'document.querySelector("[data-copy=\\"configPath\\"]").getAttribute("aria-label") === "Copy config path" && document.querySelector("#configPath").closest("button") === null && document.querySelector("#configPath").closest(".detail-path-value") !== null', 'config path is selectable text with a separate copy affordance');
    await assertEval(cdp, 'document.querySelector("[data-copy=\\"logPath\\"]").getAttribute("aria-label") === "Copy log path" && document.querySelector("#logPath").closest("button") === null && getComputedStyle(document.querySelector("#logPath")).userSelect !== "none" && getComputedStyle(document.querySelector("#logPath")).whiteSpace !== "nowrap" && getComputedStyle(document.querySelector("#logPath")).textOverflow !== "ellipsis"', 'log path is selectable text with a separate copy affordance');
