...
test('deployment spec documents Office tool E2E report validation command', () => {
  ...
  assert.match(roadmap, /activator identity/i);
  assert.match(roadmap, /activation_path/);
  assert.match(roadmap, /weak activation proof/i);
  assert.match(roadmap, /deleted_paths/);
  assert.match(roadmap, /concrete cleanup paths/i);
  assert.match(toolSpec, /deleted_paths/);
  assert.match(toolSpec, /driver-owned files were closed and deleted/i);
});
...
test('runtime evidence validator can require product visual evidence', () => {
  ...
  withEvidenceFile(ui, (uiPath) => {
    withProductVisualEvidence(true, (visualPath) => {
      const broken = JSON.parse(readFileSync(visualPath, 'utf8')) as ReturnType<typeof productVisualReport>;
      broken.screenshots_exist.tray_native_menu = false;
      writeFileSync(visualPath, JSON.stringify(broken, null, 2));
      const result = runValidator(uiPath, '--ui', '--require-product-visual', '--product-visual-evidence-path', visualPath);
      assert.notEqual(result.status, 0);
      assert.match(outputText(result.stdout), /Product visual evidence tray_native_menu screenshot ready flag does not match embedded manual tray evidence/);
    });
  });
  ...
  withEvidenceFile(ui, (uiPath) => {
    withProductVisualEvidence(true, (visualPath) => {
      const broken = JSON.parse(readFileSync(visualPath, 'utf8')) as ReturnType<typeof productVisualReport>;
      broken.manual_tray_evidence.screenshot_path = broken.screenshot_paths.word_taskpane_title;
      writeFileSync(visualPath, JSON.stringify(broken, null, 2));
      const result = runValidator(uiPath, '--ui', '--require-product-visual', '--product-visual-evidence-path', visualPath);
      assert.notEqual(result.status, 0);
      assert.match(outputText(result.stdout), /Embedded manual tray evidence screenshot path does not match tray_icon surface screenshot/);
    });
  });
  ...
  withEvidenceFile(ui, (uiPath) => {
    withProductVisualEvidence(true, (visualPath) => {
      const broken = JSON.parse(readFileSync(visualPath, 'utf8')) as ReturnType<typeof productVisualReport>;
      broken.screenshots_exist.daemon_main_window = false;
      broken.passed = false;
      writeFileSync(visualPath, JSON.stringify(broken, null, 2));
      const result = runValidator(uiPath, '--ui', '--require-product-visual', '--product-visual-evidence-path', visualPath);
      assert.notEqual(result.status, 0);
      assert.match(outputText(result.stdout), /screenshot ready flag is false: daemon_main_window/);
    });
  });
  ...
  withEvidenceFile(ui, (uiPath) => {
    withProductVisualEvidence(true, (visualPath) => {
      const broken = JSON.parse(readFileSync(visualPath, 'utf8')) as ReturnType<typeof productVisualReport>;
      broken.tray_tooltip = 'Office MCP Control - Degraded - 0 clients - 0 documents';
      broken.tray_tooltip_ready = false;
      broken.passed = false;
      writeFileSync(visualPath, JSON.stringify(broken, null, 2));
      const result = runValidator(uiPath, '--ui', '--require-product-visual', '--product-visual-evidence-path', visualPath);
      assert.notEqual(result.status, 0);
      assert.match(outputText(result.stdout), /Product visual evidence tray tooltip does not match embedded manual tray evidence/);
    });
  });
  ...
  withEvidenceFile(ui, (uiPath) => {
    withProductVisualEvidence(true, (visualPath) => {
      const broken = JSON.parse(readFileSync(visualPath, 'utf8')) as ReturnType<typeof productVisualReport>;
      broken.daemon_context = manualTrayDaemonContext(['Status: Degraded', 'Clients: 0', 'Documents: 0', '---', 'Show Office MCP Control', 'Quit Office MCP Control']);
      writeFileSync(visualPath, JSON.stringify(broken, null, 2));
      const result = runValidator(uiPath, '--ui', '--require-product-visual', '--product-visual-evidence-path', visualPath);
      assert.notEqual(result.status, 0);
      assert.match(outputText(result.stdout), /Product visual evidence daemon context does not match embedded manual tray evidence/);
    });
  });
